...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns="http://www.tei-c.org/ns/1.0" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:p="http://schema.primaresearch.org/PAGE/gts/pagecontent/2019-07-15" xmlns:mets="http://www.loc.gov/METS/" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:local="local" xmlns:xstring="https://github.com/dariok/XStringUtils" exclude-result-prefixes="#all" version="3.0">
    ...
    <xd:doc>
        <xd:desc>If false(), region types that correspond to valid TEI elements will be returned as this element; types that do not correspond to a TEI element will be returned as tei:ab[@type]. If set to true(), all region types (except for paragraph, heading) will be returned as tei:ab.</xd:desc>
    </xd:doc>
    <xsl:param name="ab" select="false()"/>

    <xd:doc>
        <xd:desc>If true(), export the (estimated) word coordinates to the facsimile section. Default: false().</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Whether to export custom attributes from tags that we do not know how to convert to valid TEI (true(), default) or whether to discard them (false()).</xd:desc>
    </xd:doc>
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p>Austrian Centre for Digital Humanities http://acdh.oeaw.ac.at | University and State Library Darmstadt https://ulb.tu-darmstadt.de</xd:p>
            <xd:p/>
            <xd:p>This stylesheet, when applied to mets.xml of the PAGE output, will create (valid) TEI</xd:p>
            <xd:p>While this XSLT is designed to run on many different flavours of PAGE-XMLs described by a common mets.xml file, some special care was taken to include meta data provided by Transkribus; other meta data providers may be included if examples are provided.</xd:p>
            <xd:p/>
    ...
    <xsl:template match="/mets:mets">
        <TEI>
                ...
                <fileDesc>
                    ...
                    <titleStmt>
                        <xsl:choose>
                            ...
                            <xsl:when test="mets:amdSec and $title = ''">
            ...
            <text>
                ...
                <body>
                    <xsl:for-each-group select="$make_div//*[local-name() = 'div']/*" group-starting-with="
    ...
    <xd:doc>
        <xd:desc>The author as stated in Transkribus meta data. Will be used in the teiHeader as titleStmt/author</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>The author as stated in Transkribus meta data. Will be used in the teiHeader as titleStmt/respStmt</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>The name of the collection from which this document was exported. Will be used as seriesStmt/title</xd:desc>
    </xd:doc>
    ...
    <xsl:template match="p:Page" mode="facsimile">
        ...
        <surface ulx="0" uly="0" lrx="{@imageWidth}" lry="{@imageHeight}" xml:id="facs_{$numCurr}">
            ...
            <graphic url="{encode-for-uri(@imageFilename)}" width="{@imageWidth}px" height="{@imageHeight}px"/>
    ...
    <xsl:template match="p:PrintSpace | p:TextRegion | p:SeparatorRegion | p:GraphicRegion | p:TextLine" mode="facsimile">
        ...
        <zone ulx="{substring-before(tokenize(p:Baseline/@points, ' ')[1], ',')}" uly="{substring-after(tokenize(p:Baseline/@points, ' ')[1], ',')}" points="{p:Baseline/@points}" rendition="{$renditionValue}">
    ...
    <xsl:template match="p:Page" mode="text">
        ...
        <xsl:apply-templates select="p:TextRegion | p:SeparatorRegion | p:GraphicRegion | p:TableRegion" mode="text">
            <xsl:with-param name="center" tunnel="true" select="number(@imageWidth) div 2" as="xs:double"/>
        </xsl:apply-templates>
    </xsl:template>
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p>PAGE labels for text region see: https://www.primaresearch.org/tools/PAGELibraries caption observed header observed footer observed page-number observed drop-capital ignored credit ignored floating ignored signature-mark observed catch-word observed marginalia observed footnote observed footnote-continued observed endnote ignored TOC-entry ignored list-label ignored other observed </xd:p>
        </xd:desc>
        <xd:param name="numCurr"/>
    ...
    <xsl:template match="p:TextRegion" mode="text">
        ...
        <xsl:choose>
            ...
            <xsl:when test="'paragraph' = $regionType or 'p' = $regionType">
                <xsl:text>
                    </xsl:text>
                ...
                <xsl:for-each select="$prepped/node()">
                    <xsl:choose>
                        <xsl:when test="@pos = 'e'">
                            ...
                            <xsl:variable name="precs" select="preceding-sibling::local:m[@pos = 's' and preceding-sibling::local:m[@o = $o]]"/>
    ...
    <xd:doc>
        <xd:desc>TranskribusMetadata contains the link to the image on Transkribus’ servers; return a tei:graphic element with this URL so it can be evaluated during postprocessing</xd:desc>
    </xd:doc>
    <xsl:template match="*:TranskribusMetadata">
        <xsl:text>
            </xsl:text>
        <graphic url="{@imgUrl}" width="{following::p:Page/@imageWidth}px" height="{following::p:Page/@imageHeight}px"/>
    </xsl:template>
